...
void serialTask(void *p) {
	...
	for (;;) {
		if (Serial.available()) {
			...
			if (output & B00010000) {
				xTaskCreate(audio2Task, "left", STACK_SIZE, NULL, 5, NULL);
			}
...
void leftMotorTask(void *p) {
	...
	for (;;) {
		...
		if (!move) {
			...
			digitalWrite(5, LOW);
			digitalWrite(6, HIGH);
			vTaskDelayUntil(&xLastWakeUpTime, 8 + onPeriod);
			digitalWrite(6, LOW);
			vTaskDelayUntil(&xLastWakeUpTime, 20 - (8 + onPeriod));
		} else {
...
void audioTask(void *p) {
	TickType_t xLastWakeTime = xTaskGetTickCount();
	int isCompleted = 0;
	int output = 0; //completion state of challenge
	for (;;) {
		for (int i = 0; i < 3; i++) {
...
void loop() {
	if (Serial.available()) {
		...
		xTaskCreate(serialTask, "serial", STACK_SIZE, NULL, 3, NULL);

		xTaskCreate(ledTask, "led", STACK_SIZE, NULL, 2, NULL);
